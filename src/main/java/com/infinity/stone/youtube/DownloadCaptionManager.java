...
public class DownloadCaptionManager {
	...
	private final List<String> SCOPES = Lists.newArrayList("https://www.googleapis.com/auth/youtube.force-ssl");
...
//		}
	...
	public SubtitleCollection downloadSubtitle(String videoId) {
		...
		if (isSuccessDownloadSubtitle) {
			subtitleCollection = buildSubtitleCollectionFromFileTTML(PATH + videoId + EXTENSION_TTML);
		}
	...
	private CaptionsModel getCaptions(String videoId) throws IOException {
		CaptionListResponse captionListResponse = mYouTube.captions().list("snippet", videoId).execute();
	...
	private void downloadCaption(String captionId, String fileName) throws IOException {
		Download captionDownload = mYouTube.captions().download(captionId).setTfmt(TTML);
