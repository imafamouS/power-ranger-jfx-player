...
public class AwtImageUtilities {
    ...
    /**
     * Progressive bilinear scaling: for any downscale size, scale iteratively by halves using
     * BILINEAR filtering until the proper size is reached.
     */
    public static BufferedImage getOptimalScalingImage(BufferedImage inputImage, int width,
              int height) {
        ...
        while (currentWidth > 1 && currentHeigth > 1) {
            if (deltaX <= nextPow2X || deltaY <= nextPow2Y) {
                if (currentWidth != width || currentHeigth != height) {
                    ...
                    g.drawImage(currentImage, 0, 0, tmpImage.getWidth(), tmpImage.getHeight(),
                              null);
                    currentImage = tmpImage;
                }
                return currentImage;
            } else {
                BufferedImage tmpImage = createCompatibleImage(currentWidth >> 1,
                          currentHeigth >> 1, true);
                Graphics g = tmpImage.getGraphics();
                ((Graphics2D) g).setRenderingHint(RenderingHints.KEY_INTERPOLATION,
