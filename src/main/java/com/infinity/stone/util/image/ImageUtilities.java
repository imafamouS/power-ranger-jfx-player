...
public class ImageUtilities {
    ...
    public static Image fastResize(Image image, int width, int height) {
        if (width <= 0 || height <= 0) {
            throw new IllegalArgumentException(
                      "Dimension must be > 0 (" + width + "/" + height + ")");
        }
        ...
        while (xToWrite < width) {
            ...
            while (yToWrite < height) {
                writer.setArgb(xToWrite, yToWrite, reader.getArgb((int) (xToWrite * widthFactor),
                          (int) (yToWrite * heightFactor)));
                yToWrite++;
            }
            xToWrite++;
        }
    ...
    public static Image resizeByAwt(Image image, int width, int height, InterpolationType type) {
        ...
        BufferedImage resizedAwtImage = AwtImageUtilities
                  .createCompatibleImage(width, height, true);
    ...
    public static Image progressiveResize(Image image, int width, int height,
        ...
        if (image.getWidth() * 2 < width && image.getHeight() * 2 < height) {
            Image resizedImage = fastResize(image, (int) image.getWidth() * 2,
    ...
    public static Image filter(Image image, BufferedImageOp filter) {
        BufferedImage awtImage = SwingFXUtils.fromFXImage(image, null);
        BufferedImage filteredAwtImage = AwtImageUtilities
                  .createCompatibleImage(awtImage.getWidth(), awtImage.getHeight(), true);
    ...
    public static Image clipImage(Image image, Rectangle2D clip) {
        WritableImage clippedImage = new WritableImage((int) clip.getWidth(),
                  (int) clip.getHeight());
        int[] rgbaValues = getRGB(image, (int) clip.getMinX(), (int) clip.getMinY(),
    ...
    public static Image fitInDimensionAndCutEdges(Image image, double width, double heigth) {
        Dimension2D newImageDimension = UiUtilities
                  .shouldFitIn(image.getWidth(), image.getHeight(), width, heigth);
        Image resizedImage = fastResize(image, newImageDimension);
        return clipImage(resizedImage, new Rectangle2D((resizedImage.getWidth() - width) / 2,
                  (resizedImage.getHeight() - heigth) / 2, width, heigth));
    }
