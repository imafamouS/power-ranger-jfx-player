...
public class VideoShowController implements Initializable,
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        favouriteRepo = (FavoriteRepository) RepositoryManager
                .getInstance(RepositoryType.FAVORITE);
        ...
        subtitleRepository.findAllSubtitleByVideoId(Constant.CURRENT_VIDEO_ID)
        			.doOnSuccess((result)->{
        				favouriteRepo.findFavoritesByVideoId(Constant.CURRENT_VIDEO_ID)
            			.subscribe(success->{
            				mFavoriteSubtitleCollection =
            						SubtitleCollection.makeSubtitleCollectionFromListFavorite(success, result);           			
            			},throwable->{           	
            			});
        			})
                  .subscribe(success -> {
    ...
    private void setUpFavoriteSubTitle() {
    	...
    	favouriteListview.setCellFactory((Callback<JFXListView<Subtitle>, JFXListCell<Subtitle>>) param ->
                            new ListViewFavouriteCell(sub->{
                            	 mFavoriteRepository.delete(new Favorite(sub.getVideoId(), sub.getId()))
                                 .subscribe(success -> {
                                     JFXSnackbar snackbar = new JFXSnackbar(rightPaneSide);
                                     snackbar.show("Remove favorite successfully", 1000);
                                     TrackingManager.getInstance()
                                               .track(Action.DELETE_FAVORITE,sub.getContent());
                                 }, throwable -> {
    ...
    static class ListViewFavouriteCell extends JFXListCell<Subtitle> {
        ...
        public ListViewFavouriteCell(OnClickFavorite onClickFavorite, JFXListView listFavoriteSubtitle) {
